...
func (ae *CoreTestExecutor) executeTx(txIndex string, tx *mj.Transaction) (*vmi.VMOutput, error) {
...
func (ae *CoreTestExecutor) senderHasEnoughBalance(tx *mj.Transaction) bool {
	if !tx.Type.HasSender() {
		return true
	}
...
func (ae *CoreTestExecutor) simpleTransferOutput(tx *mj.Transaction) (*vmi.VMOutput, error) {
...
func (ae *CoreTestExecutor) validatorRewardOutput(tx *mj.Transaction) (*vmi.VMOutput, error) {
	...
	storageDharitriReward := recipient.StorageValue(DharitriRewardKey)
	storageDharitriReward = big.NewInt(0).Add(
		big.NewInt(0).SetBytes(storageDharitriReward),
		reward).Bytes()
	...
	outputAccounts[string(tx.To.Value)] = &vmcommon.OutputAccount{
		...
		StorageUpdates: map[string]*vmi.StorageUpdate{
			DharitriRewardKey: &vmi.StorageUpdate{
				Offset: []byte(DharitriRewardKey),
				Data:   storageDharitriReward,
			},
		},
	}
...
func (ae *CoreTestExecutor) scCreate(txIndex string, tx *mj.Transaction) (*vmi.VMOutput, error) {
...
func (ae *CoreTestExecutor) scCall(txIndex string, tx *mj.Transaction) (*vmi.VMOutput, error) {
	recipient := ae.World.AcctMap.GetAccount(tx.To.Value)
	if recipient == nil {
		return nil, fmt.Errorf("Tx recipient (address: %s) does not exist", hex.EncodeToString(tx.To.Value))
	}
...
func (ae *CoreTestExecutor) updateStateAfterTx(
	tx *mj.Transaction,
