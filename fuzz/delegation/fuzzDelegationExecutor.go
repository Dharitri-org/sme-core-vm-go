...
import (
	...
	am "github.com/Dharitri-org/sme-core-vm-go/coremandos"
...
type fuzzDelegationExecutor struct {
	coreTestExecutor *am.CoreTestExecutor
...
func newFuzzDelegationExecutor(fileResolver mjparse.FileResolver) (*fuzzDelegationExecutor, error) {
	coreTestExecutor, err := am.NewCoreTestExecutor()
	if err != nil {
		return nil, err
	}
	...
	return &fuzzDelegationExecutor{
		coreTestExecutor:    coreTestExecutor,
		world:               coreTestExecutor.World,
		vm:                  coreTestExecutor.GetVM(),
		mandosParser:        parser,
		txIndex:             0,
		numNodes:            0,
...
func (pfe *fuzzDelegationExecutor) executeStep(stepSnippet string) error {
	...
	pfe.addStep(step)
	return pfe.coreTestExecutor.ExecuteStep(step)
}
...
func (pfe *fuzzDelegationExecutor) executeTxStep(stepSnippet string) (*vmi.VMOutput, error) {
	...
	return pfe.coreTestExecutor.ExecuteTxStep(txStep)
}
