...
func CreateGasConfig(gasMap GasScheduleMap) (*GasCost, error) {
	...
	dharitriOps := &DharitriAPICost{}
	err = mapstructure.Decode(gasMap["DharitriAPICost"], dharitriOps)
	if err != nil {
		return nil, err
	}

	err = checkForZeroUint64Fields(*dharitriOps)
	if err != nil {
		return nil, err
	}
...
func FillGasMap(gasMap GasScheduleMap, value, asyncCallbackGasLock uint64) GasScheduleMap {
	...
	gasMap["DharitriAPICost"] = FillGasMap_DharitriAPICosts(value, asyncCallbackGasLock)
...
func FillGasMap_DharitriAPICosts(value, asyncCallbackGasLock uint64) map[string]uint64 {
	gasMap := make(map[string]uint64)
