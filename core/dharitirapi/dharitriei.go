...
// extern int32_t getDCTValue(void *context, int32_t resultOffset);
// extern int32_t getDCTTokenName(void *context, int32_t resultOffset);
...
func DharitriEIImports() (*wasmer.Imports, error) {
	...
	imports, err = imports.Append("getDCTValue", getDCTValue, C.getDCTValue)
	if err != nil {
		return nil, err
	}

	imports, err = imports.Append("getDCTValue", getDCTTokenName, C.getDCTTokenName)
	if err != nil {
		return nil, err
	}
...
func transferValue(context unsafe.Pointer, destOffset int32, valueOffset int32, dataOffset int32, length int32) int32 {
	...
	valueBytes, err := runtime.MemLoad(valueOffset, core.BalanceLen)
	if core.WithFault(err, context, runtime.DharitriAPIErrorShouldFailExecution()) {
		return 1
	}
...
func upgradeContract(
	...
	codeMetadata, err := runtime.MemLoad(codeMetadataOffset, core.CodeMetadataLen)
	if core.WithFault(err, context, runtime.DharitriAPIErrorShouldFailExecution()) {
		return
	}
	...
	finalData := core.UpgradeFunctionName + "@" + codeEncoded + "@" + codeMetadataEncoded
...
//export getDCTValue
func getDCTValue(context unsafe.Pointer, resultOffset int32) int32 {
	runtime := core.GetRuntimeContext(context)
	metering := core.GetMeteringContext(context)
	...
	dctValue := runtime.GetVMInput().DCTValue
	if dctValue != nil {
		value = dctValue.Bytes()
		value = core.PadBytesLeft(value, core.BalanceLen)
	}
...
//export getDCTTokenName
func getDCTTokenName(context unsafe.Pointer, resultOffset int32) int32 {
	runtime := core.GetRuntimeContext(context)
	metering := core.GetMeteringContext(context)
	...
	tokenName := runtime.GetVMInput().DCTTokenName
...
func getCallValueTokenName(context unsafe.Pointer, callValueOffset int32, tokenNameOffset int32) int32 {
	...
	tokenName := []byte("mOAX")
	if len(runtime.GetVMInput().DCTTokenName) > 0 {
		tokenName = runtime.GetVMInput().DCTTokenName
		callValue = runtime.GetVMInput().DCTValue.Bytes()
	}
	callValue = core.PadBytesLeft(callValue, core.BalanceLen)
