...
func TestNewCore(t *testing.T) {
	host, err := DefaultTestCore(t, &mock.BlockchainHookStub{})
	require.Nil(t, err)
...
func TestExecution_DeployOutOfGas(t *testing.T) {
	...
	host := DefaultTestCoreForDeployment(t, 24, newAddress)
	input := DefaultTestContractCreateInput()
	...
	require.Equal(t, core.ErrNotEnoughGas.Error(), vmOutput.ReturnMessage)
}
...
func TestExecution_CallGetUserAccountErr(t *testing.T) {
	...
	require.Equal(t, core.ErrContractNotFound.Error(), vmOutput.ReturnMessage)
}
...
func TestExecution_CallSCMethod(t *testing.T) {
	...
	require.NotNil(t, vmOutput)
	require.Equal(t, vmcommon.UserError, vmOutput.ReturnCode)
	require.Equal(t, core.ErrInitFuncCalledInRun.Error(), vmOutput.ReturnMessage)
	...
	require.NotNil(t, vmOutput)
	require.Equal(t, vmcommon.UserError, vmOutput.ReturnCode)
	require.Equal(t, core.ErrCallBackFuncCalledInRun.Error(), vmOutput.ReturnMessage)
...
func TestExecution_Call_Successful(t *testing.T) {
	code := GetTestSCCode("counter", "../../")
	host, stubBlockchainHook := DefaultTestCoreForCall(t, code, nil)
...
func TestExecution_ExecuteOnSameContext_Simple(t *testing.T) {
	...
	host, _ := DefaultTestCoreForTwoSCs(t, parentCode, childCode, nil)
	input := DefaultTestContractCallInput()
...
func TestExecution_ExecuteOnSameContext_Prepare(t *testing.T) {
	...
	host, _ := DefaultTestCoreForCall(t, parentCode, parentSCBalance)
	input := DefaultTestContractCallInput()
	input.RecipientAddr = parentAddress
...
func TestExecution_ExecuteOnSameContext_OutOfGas(t *testing.T) {
	...
	// end in OutOfGas.
	host, _ := DefaultTestCoreForTwoSCs(t, parentCode, childCode, parentSCBalance)
	input := DefaultTestContractCallInput()
	input.RecipientAddr = parentAddress
...
func TestExecution_ExecuteOnSameContext_Recursive_Direct(t *testing.T) {
	...
	host, _ := DefaultTestCoreForCall(t, code, scBalance)
	input := DefaultTestContractCallInput()
	input.RecipientAddr = parentAddress
...
func TestExecution_ExecuteOnSameContext_BuiltinFunctions(t *testing.T) {
	...
	host, stubBlockchainHook := DefaultTestCoreForCall(t, code, scBalance)
...
func TestExecution_AsyncCall_ChildFails(t *testing.T) {
	...
	// SC and pass some arguments using executeOnDestContext().
	host, _ := DefaultTestCoreForTwoSCs(t, parentCode, childCode, parentSCBalance)
	host.Metering().GasSchedule().DharitriAPICost.AsyncCallbackGasLock = 3000
...
func TestExecution_CreateNewContract_Success(t *testing.T) {
	...
	host, stubBlockchainHook := DefaultTestCoreForCall(t, parentCode, parentBalance)
...
func makeBytecodeWithLocals(numLocals uint64) []byte {
	...
	encodedNumLocals := core.U64ToLEB128(numLocals)
