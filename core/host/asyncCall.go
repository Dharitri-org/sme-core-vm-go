...
import (
	...
	"github.com/Dharitri-org/sme-core-vm-go/math"
...
func (host *vmHost) handleAsyncCallBreakpoint() error {
	...
	runtime.SetRuntimeBreakpointValue(core.BreakpointNone)
...
func (host *vmHost) determineAsyncCallExecutionMode(asyncCallInfo *core.AsyncCallInfo) (core.AsyncCallExecutionMode, error) {
	runtime := host.Runtime()
	...
	if host.isBuiltinFunctionName(functionName) {
		...
		return core.AsyncBuiltinFunc, nil
	}

	return core.AsyncUnknown, nil
}

func (host *vmHost) executeSyncDestinationCall(asyncCallInfo *core.AsyncCallInfo) (*vmcommon.VMOutput, error) {
...
func (host *vmHost) sendAsyncCallToDestination(asyncCallInfo core.AsyncCallInfoHandler) error {
	runtime := host.Runtime()
	output := host.Output()
...
func (host *vmHost) createDestinationContractCallInput(asyncCallInfo core.AsyncCallInfoHandler) (*vmcommon.ContractCallInput, error) {
	runtime := host.Runtime()
	...
	gasToUse := host.Metering().GasSchedule().DharitriAPICost.AsyncCallStep
	if gasLimit <= gasToUse {
		return nil, core.ErrNotEnoughGas
	}
	gasLimit -= gasToUse
...
func (host *vmHost) createCallbackContractCallInput(
	...
	gasToUse := metering.GasSchedule().DharitriAPICost.AsyncCallStep
 ...
 */
func (host *vmHost) processAsyncInfo(asyncInfo *core.AsyncContextInfo) (*core.AsyncContextInfo, error) {
 ...
 */
func (host *vmHost) processAsyncCall(asyncCall *core.AsyncGeneratedCall) error {
 ...
 */
func (host *vmHost) callbackAsync(asyncCall *core.AsyncGeneratedCall, vmOutput *vmcommon.VMOutput, executionError error) error {
	asyncCall.Status = core.AsyncCallResolved
	callbackFunction := asyncCall.SuccessCallback
	if vmOutput.ReturnCode != vmcommon.Ok {
		asyncCall.Status = core.AsyncCallRejected
 ...
 */
func (host *vmHost) savePendingAsyncCalls(pendingAsyncMap *core.AsyncContextInfo) error {
	...
	asyncCallStorageKey := core.CustomStorageKey(core.AsyncDataPrefix, runtime.GetOriginalTxHash())
 ...
 */
func (host *vmHost) saveCrossShardCalls(asyncInfo *core.AsyncContextInfo) error {
	crossMap := &core.AsyncContextInfo{
	...
	for contextIdentifier, asyncContext := range asyncInfo.AsyncContextMap {
		for _, asyncCall := range asyncContext.AsyncCalls {
			if !host.canExecuteSynchronously(asyncCall.Destination, asyncCall.Data) {
				...
				if !ok {
					crossMap.AsyncContextMap[contextIdentifier] = &core.AsyncContext{
 ...
 */
func (host *vmHost) getPendingAsyncCalls(asyncInfo *core.AsyncContextInfo) *core.AsyncContextInfo {
	pendingMap := &core.AsyncContextInfo{
		CallerAddr:      asyncInfo.CallerAddr,
		ReturnData:      asyncInfo.ReturnData,
		AsyncContextMap: make(map[string]*core.AsyncContext),
	}
	...
	for contextIdentifier, asyncContext := range asyncInfo.AsyncContextMap {
		for _, asyncCall := range asyncContext.AsyncCalls {
			if asyncCall.Status != core.AsyncCallPending {
				continue
			}

			_, ok := pendingMap.AsyncContextMap[contextIdentifier]
			if !ok {
				pendingMap.AsyncContextMap[contextIdentifier] = &core.AsyncContext{
					Callback:   asyncContext.Callback,
					AsyncCalls: make([]*core.AsyncGeneratedCall, 0),
				}
			}
...
func (host *vmHost) processCallbackStack() error {
	...
	if len(currentContextIdentifier) == 0 {
		return core.ErrCallBackFuncNotExpected
	}
 ...
 */
func (host *vmHost) setupAsyncCallsGas(asyncInfo *core.AsyncContextInfo) error {
...
func (host *vmHost) getCurrentAsyncInfo() (*core.AsyncContextInfo, error) {
	runtime := host.Runtime()
	storage := host.Storage()
