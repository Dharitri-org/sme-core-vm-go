...
var _ core.OutputContext = (*outputContext)(nil)
...
func NewOutputContext(host core.VMHost) (*outputContext, error) {
...
func (context *outputContext) TransferValueOnly(destination []byte, sender []byte, value *big.Int) error {
	if value.Cmp(core.Zero) < 0 {
		return core.ErrTransferNegativeValue
	}
...
func (context *outputContext) DeployCode(input core.CodeDeployInput) {
...
func (context *outputContext) resolveReturnCodeFromError(err error) vmcommon.ReturnCode {
	...
	if errors.Is(err, core.ErrSignalError) {
		return vmcommon.UserError
	}
	if errors.Is(err, core.ErrFuncNotFound) {
		return vmcommon.FunctionNotFound
	}
	if errors.Is(err, core.ErrFunctionNonvoidSignature) {
		return vmcommon.FunctionWrongSignature
	}
	if errors.Is(err, core.ErrInvalidFunction) {
		return vmcommon.UserError
	}
	if errors.Is(err, core.ErrNotEnoughGas) {
		return vmcommon.OutOfGas
	}
	if errors.Is(err, core.ErrContractNotFound) {
		return vmcommon.ContractNotFound
	}
	if errors.Is(err, core.ErrContractInvalid) {
		return vmcommon.ContractInvalid
	}
	if errors.Is(err, core.ErrUpgradeFailed) {
		return vmcommon.UpgradeFailed
	}
	if errors.Is(err, core.ErrTransferInsufficientFunds) {
