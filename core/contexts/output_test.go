...
func TestOutputContext_GetOutputAccount(t *testing.T) {
	...
	require.Equal(t, core.Zero, account.BalanceDelta)
...
func TestOutputContext_VMOutputError(t *testing.T) {
	...
	returnMessage := core.ErrContractNotFound.Error()
	...
	vmOutput := outputContext.CreateVMOutputInCaseOfError(core.ErrContractNotFound)
...
func TestOutputContext_Transfer_Errors_And_Checks(t *testing.T) {
	...
	require.Equal(t, core.Zero, senderOutputAccount.BalanceDelta)

	// negative transfers are disallowed
	valueToTransfer := big.NewInt(-1000)
	err := outputContext.Transfer(receiver, sender, 54, valueToTransfer, []byte("txdata"))
	require.Equal(t, core.ErrTransferNegativeValue, err)
	require.Nil(t, senderOutputAccount.Balance)
	require.Equal(t, core.Zero, senderOutputAccount.BalanceDelta)
	...
	err = outputContext.Transfer(receiver, sender, 54, valueToTransfer, []byte("txdata"))
	require.Equal(t, core.ErrTransferInsufficientFunds, err)
...
func TestOutputContext_Transfer_IsAccountPayable(t *testing.T) {
	...
	err := oc.Transfer(receiverNonPayable, sender, 54, valueToTransfer, []byte("txdata"))

	require.Equal(t, core.ErrAccountNotPayable, err)
	...
	valueToTransfer = big.NewInt(10)
	err = oc.Transfer(receiverPayable, sender, 54, valueToTransfer, []byte("txdata"))
